        ...
        <div className="p-[10px] pt-[25px] h-[85%] w-full flex flex-col gap-[10px]">
            ...
            <>
              <NavLink key={val.id}
                  ...
                  <button className={`rounded-full p-4 border-[5px] ${location.pathname === val.loc ? 'border-[--primary] scale-[1.2] ' : ''}`}>
                      <val.icon/>
                  </button>
                  <div className="flex flex-col relative w-full ">
                          <p className="text-[22px] ml-5 text-[--secondary] w-full">{val.name}</p>
                          <p className="text-[12px] ml-7 text-[#9f9f9f] absolute top-[26px] w-full">{val.desc}</p>
                     </div>    
            </NavLink>
            </>
          );})}
